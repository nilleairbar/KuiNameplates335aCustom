...
--[[ 	--self:RegisterEvent("NAME_PLATE_CREATED")
	...
	function addon:NAME_PLATE_OWNER_CHANGED(event, unit)
		addon:NAME_PLATE_UNIT_REMOVED(nil, unit)
		addon:NAME_PLATE_UNIT_ADDED(nil, unit)
	end
	function addon:NAME_PLATE_UNIT_REMOVED(event, unit)
		...
		if plate and addon:IsNameplate(plate) and plate.kui then
			for i, p in ipairs(addon.frameList) do
				if p == plate then
					tremove(addon.frameList, i)
					break
				end
			end
		end
